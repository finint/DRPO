...
class DRPOTrainer():
    ...
    def _load_data(self):
        ...
        self.data_loader = torch.utils.data.DataLoader( dataset=dataset, batch_size=self.config["batch_size"], sampler=train_sampler)

    def _init_tb(self):
        time = datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
        device_name = f"{self.device}".split(":")
        device_name = device_name[0]+"_"+device_name[1]
